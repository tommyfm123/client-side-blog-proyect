...
export default function Header() {
  ...
  const [scrolling, setScrolling] = useState(false); // Estado para controlar si se hace scroll
  const [lastScrollTop, setLastScrollTop] = useState(0); // Estado para controlar la última posición del scroll
  const [direction, setDirection] = useState("up"); // Dirección del scroll ('up' o 'down')
  const [initialRender, setInitialRender] = useState(true); // Control del primer renderizado
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (currentScrollTop > lastScrollTop) {
        setDirection("down"); // Hacia abajo
      } else {
        setDirection("up"); // Hacia arriba
      }
      setLastScrollTop(currentScrollTop <= 0 ? 0 : currentScrollTop); // Evita que se vuelva negativo
      setScrolling(currentScrollTop > 50); // Cambia el estado al hacer scroll
    };
  ...
  }, [lastScrollTop, setUserInfo]);

  useEffect(() => {
    // Asegurar que el header sea visible al cargar la página
    const timer = setTimeout(() => {
      setInitialRender(false); // Cambia el estado después de un breve retraso
    }, 300); // Retraso opcional para un efecto de carga suave
  ...
  return (
    <header
      className={`${scrolling ? "scrolled" : ""} ${
        initialRender ? "visible" : direction === "down" ? "hidden" : "visible"
      }`}
    >
